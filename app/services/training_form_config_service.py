...
def list_configurations_service(db: Session) -> list[dict]:
    rows = db.query(TrainingFormConfiguration).order_by(TrainingFormConfiguration.created_at.desc()).all()
...
def put_assignments_service(db: Session, config_id: UUID, payload, current_user: dict) -> dict:
    ...
    # Empty enterprise_ids list ⇒ global
    if payload.enterprise_ids is not None and len(payload.enterprise_ids) == 0:
